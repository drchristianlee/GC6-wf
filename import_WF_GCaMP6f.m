...
mouse = 'R:\Margolis Lab Server\DATA\GCaMP6f WF\180413\for David\fD1\whisker';
...
dim = [100 100]; % dimension of the movie in pixels x,y
Ca.sample_rate = 100;    %in frames / sec

cd(mouse); % start in session directory, e.g. thin01-17

% get list of all .tif files (32-bit DF/F) in folder
% files = 'DFF\*.tif';
files = [img_type ch '\*.tif'];
files = dir(files);
...
for movie = 1:size(files,1) % files_list,2 was files,1
    ...
    for r = 1:length(refRoi)
        ...
        Ca.Ch0{r,movie} = extracted_ch0; % each sweep for each roi
    end
